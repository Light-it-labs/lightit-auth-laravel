...
describe('two-factor account management stub rendering', function () use ($fixturePath, $stubPath): void {
    it('renders each screen byte-for-byte against its golden fixture', function (
        ...
        [
            '_private/2fa/account/page.tsx.stub',
            '_private/2fa/account/page.tsx',
        ],
        [
            '_private/2fa/account/-components/disable-two-factor-dialog.tsx.stub',
            '_private/2fa/account/-components/disable-two-factor-dialog.tsx',
        ],
        [
            '_private/2fa/account/-components/request-two-factor-reset-dialog.tsx.stub',
            '_private/2fa/account/-components/request-two-factor-reset-dialog.tsx',
        ],
        [
            '_private/2fa/account/-components/regenerate-recovery-codes-dialog.tsx.stub',
            '_private/2fa/account/-components/regenerate-recovery-codes-dialog.tsx',
        ],
        [
    ...
    it(
        'never reads the real session accessToken directly - only through a token prop',
    ...
    it('sends the reset challenge token through the URL, not through session.ts', function () use ($stubPath): void {
        $rendered = (new StubRenderer())->render(
            $stubPath('_private/2fa/account/-components/request-two-factor-reset-dialog.tsx.stub'),
    ...
    it('reuses the setup flow\'s recovery-codes display instead of duplicating it', function () use ($stubPath): void {
        $rendered = (new StubRenderer())->render(
            $stubPath('_private/2fa/account/-components/regenerate-recovery-codes-dialog.tsx.stub'),
            FrontendStubTokens::defaults(),
        );
    ...
    it(
        'has a standalone account-management page independent of the passkeys security page',
        function () use ($stubPath): void {
            $rendered = (new StubRenderer())->render(
                $stubPath('_private/2fa/account/page.tsx.stub'),
                FrontendStubTokens::defaults(),
            );
    
            expect($rendered)
                ->toContain('TwoFactorAccountPage')
